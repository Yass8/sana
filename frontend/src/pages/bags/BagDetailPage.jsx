      ...
      <div className="bg-white border border-slate-100 rounded-xl overflow-hidden">
        ...
        {parcels.length === 0 ? (
          ...
          <div className="flex items-center gap-6 px-5 py-3.5
                bg-slate-50 border-b border-slate-100 flex-wrap">
            {[
                { label: 'Envoi',       value: bag?.shipment?.reference },
                { label: 'Destination', value: bag?.shipment?.destinationAgency?.city },
                { label: 'Départ',      value: bag?.shipment?.departureDate
                    ? new Date(bag.shipment.departureDate).toLocaleDateString('fr-FR', {
                ...
                <p className="text-xs font-medium text-[#0F1923] mt-0.5">
                    {value ?? '—'}
                </p>
                </div>
            ))}
